...
predicate when the owner-side update protocol requires it.
...
The current implementation already allow-lists the five predicates in the first
five sidecar rows (`affectedBy`, `implementedBy`, `trackedBy`, `validatedBy`,
and `satisfiedBy`). It does not allow arbitrary RDF, which is the right safety
boundary. The configuration-management work should move this list into a
named policy/registry so that predicate ownership, whether the predicate is
generated by StrictDoc, and whether it is writable in a stream or baseline are
reviewable data rather than scattered conditionals.
