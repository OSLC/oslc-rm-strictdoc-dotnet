...
public sealed class StrictDocServiceRelationTests : IAsyncDisposable
{
    ...
    public async Task GetAllRequirements_MapsParentRolesAndTheirInverses()
    {
        ...
        var context = new ConfigurationContext("main", "HEAD", _dataDirectory);
        var service = new StrictDocService(cache, NullLogger<StrictDocService>.Instance);

        var requirements = await service.GetAllRequirementsAsync(context, BaseUrl);
